...
def _result_heading(result: ScanResult | None, ticker: str) -> str:
    ...
    status = f" · {escape(result.market_data_status)}" if result.market_data_status else ""
...
<label class="mode"><input id="fake-mode" type="checkbox" name="fake" value="1"{checked}> Modo demostración</label><button type="submit">Scan</button></form><p id="demo-label" class="demo-label"{' hidden' if not checked else ''}>Datos simulados — no proceden de Interactive Brokers</p>{table}{_summary(result)}<script>
const box=document.querySelector('#connection'), fake=document.querySelector('#fake-mode'), label=document.querySelector('#demo-label');
