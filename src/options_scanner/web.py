...
def _candidate_technical_context(candidate, context=None) -> str:
    ...
    if context and context.period == HistoricalPeriod.MULTI:
        matches = [item for item in context.confluences if item.kind.value == "support"]
        if not matches:
            return '<span class="na technical-compact">Sin confluencia</span>'
        item = min(matches, key=lambda value: min(abs(candidate.strike-value.lower),
                                                  abs(candidate.strike-value.upper)))
        relation = item.classify_strike(candidate.strike)
        ...
        return (f'<details class="candidate-technical"><summary><span class="technical-compact">'
                f'Strike {escape(relation)} de confluencia · {len(item.origins)}/3</span></summary>'
                f'<p>${item.lower:.2f}–${item.upper:.2f} · {escape(origins)}</p></details>')
