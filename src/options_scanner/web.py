...
from options_scanner.universe import (DEFAULT_GROUPS, TickerUniverseManager,
                                      UniverseSource)
...
def render_page(values: dict[str, str] | None = None, result: ScanResult | tuple[ScanResult, ...] | None = None,
                error: str | None = None, *, groups=DEFAULT_GROUPS, watchlists=()) -> bytes:
    v = {"ticker": "NVDA", "min_dte": "30", "max_dte": "45", "min_safety_margin": "20",
         ...
         "universe_source": "manual", "group": next(iter(groups), ""), "watchlist_id": ""}
    if values:
        v.update(values)
    checked = " checked" if v["mode"] == "fake" else ""
    selected_tickers = v.get("resolved_tickers", v["ticker"]).replace(",", " ").upper().split()
    universe_chips = "".join(f'<span>{escape(symbol)}</span>' for symbol in selected_tickers)
    group_options = "".join(f'<option value="{escape(name)}" data-tickers="{escape(" ".join(symbols))}"{" selected" if v["group"] == name else ""}>{escape(name)}</option>' for name, symbols in groups.items())
    watchlist_options = "".join(f'<option value="{escape(item.id)}" data-tickers="{escape(" ".join(item.symbols))}"{" selected" if v["watchlist_id"] == item.id else ""}>{escape(item.name)}</option>' for item in watchlists)
    alert = f'<div class="error" role="alert">{escape(error)}</div>' if error else ""
    results = () if result is None else result if isinstance(result, tuple) else (result,)
    table = "".join(f'''<section>{_result_heading(item, ticker)}{_technical_chart(item)}{_interpretation(item)}<h2>Candidatos completos</h2><div class="scroll"><table><thead><tr>{''.join(f'<th>{h}</th>' for h in ('Ticker','Expiration','DTE','Strike','Underlying','Safety margin','Bid','Ask','Mid','Delta','Gamma','Theta','Vega','IV','Open interest','6509','Premium yield','Annualized yield','Contexto técnico'))}</tr></thead><tbody>{_rows(item)}</tbody></table></div>{_summary(item)}</section>''' for ticker, item in zip(selected_tickers, results))
...
form{{display:flex;flex-wrap:wrap;gap:1rem;align-items:end;background:white;padding:1.25rem;border-radius:10px;box-shadow:0 2px 8px #0001}}label{{display:grid;gap:.35rem;font-weight:600}}input,select{{padding:.55rem;border:1px solid #aab3c5;border-radius:5px;width:9rem}}button{{background:#2358d5;color:white;border:0;border-radius:5px;padding:.7rem 1.4rem;font-weight:700;cursor:pointer}}button:disabled{{cursor:not-allowed;opacity:.65}}.mode{{display:flex;align-items:center;gap:.4rem}}.mode input{{width:auto}}.universe{{flex-basis:100%;display:flex;flex-wrap:wrap;gap:1rem;align-items:end;border:1px solid #d8deea;padding:1rem;border-radius:8px}}.universe h2{{flex-basis:100%;margin:0;font-size:1.05rem}}.manual input{{width:22rem}}.universe-preview{{flex-basis:100%;background:#eef3fb;padding:.75rem;border-radius:6px}}.universe-preview strong{{display:block;margin-bottom:.4rem}}.ticker-chips{{display:flex;gap:.4rem;flex-wrap:wrap}}.ticker-chips span{{background:#263451;color:white;border-radius:999px;padding:.25rem .65rem;font-weight:700}}
...
<section class="universe"><h2>Universo de tickers · una fuente</h2><label>Fuente<select name="universe_source"><option value="group"{' selected' if v['universe_source']=='group' else ''}>Grupo predefinido</option><option value="watchlist"{' selected' if v['universe_source']=='watchlist' else ''}>Mi watchlist</option><option value="manual"{' selected' if v['universe_source']=='manual' else ''}>Lista manual temporal</option></select></label><label data-source="group">Grupo<select name="group">{group_options}</select></label><label data-source="watchlist">Watchlist<select name="watchlist_id"><option value="">Selecciona…</option>{watchlist_options}</select></label><label class="manual" data-source="manual">Tickers (comas o espacios)<input name="ticker" value="{escape(v['ticker'])}"></label><label class="mode" data-source="manual"><input type="checkbox" name="save_watchlist" value="1"> Guardar como watchlist</label><label data-source="manual">Nombre<input name="watchlist_name" value=""></label><div class="universe-preview"><strong>Universo seleccionado antes del scan</strong><div id="ticker-preview" class="ticker-chips">{universe_chips}</div></div></section><label>Min DTE<input type="number" name="min_dte" min="0" value="{escape(v['min_dte'])}" required></label><label>Max DTE<input type="number" name="max_dte" min="0" value="{escape(v['max_dte'])}" required></label>
...
form.addEventListener('submit',async event=>{{event.preventDefault();if(scanning)return;scanning=true;scanButton.disabled=true;scanButton.textContent='Scan en curso...';scanOutput.replaceChildren();scanStatus.hidden=false;document.querySelector('#scan-title').textContent=form.elements.universe_source.value==='manual'?'Escaneando '+form.elements.ticker.value.trim().toUpperCase()+'...':'Escaneando '+document.querySelector('#ticker-preview').textContent.trim()+'...';document.querySelector('#scan-source').textContent=fake.checked?'Consultando datos de demostración...':'Consultando Interactive Brokers...';const started=performance.now();timer.textContent='00:00';interval=setInterval(()=>timer.textContent=elapsed((performance.now()-started)/1000),250);try{{const response=await fetch('/',{{method:'POST',body:new URLSearchParams(new FormData(form)),headers:{{'X-Requested-With':'fetch'}}}}),html=await response.text(),doc=new DOMParser().parseFromString(html,'text/html'),output=doc.querySelector('#scan-output');if(!output)throw new Error('invalid response');scanOutput.replaceChildren(...Array.from(output.childNodes).map(node=>document.importNode(node,true)));const seconds=(performance.now()-started)/1000;if(response.ok){{const done=document.createElement('p');done.className='completion';done.textContent='Scan completado en '+seconds.toFixed(1)+' s';scanOutput.prepend(done)}}else if(!scanOutput.querySelector('[role="alert"]'))throw new Error('unsafe response')}}catch(error){{scanOutput.replaceChildren();const alert=document.createElement('div');alert.className='error';alert.setAttribute('role','alert');alert.textContent='No se pudo completar el scan. Inténtalo de nuevo.';scanOutput.append(alert)}}finally{{finishScan()}}}});
function updateUniverse(){{const source=form.elements.universe_source.value;document.querySelectorAll('[data-source]').forEach(node=>node.hidden=node.dataset.source!==source);let symbols='';if(source==='manual')symbols=form.elements.ticker.value;else if(source==='group')symbols=form.elements.group.options[form.elements.group.selectedIndex]?.dataset.tickers||'';else symbols=form.elements.watchlist_id.options[form.elements.watchlist_id.selectedIndex]?.dataset.tickers||'';const parsed=symbols.toUpperCase().split(/[\\s,]+/).filter(Boolean);document.querySelector('#ticker-preview').replaceChildren(...parsed.map(symbol=>{{const chip=document.createElement('span');chip.textContent=symbol;return chip}}))}}
form.elements.universe_source.addEventListener('change',updateUniverse);form.elements.ticker.addEventListener('input',updateUniverse);form.elements.group.addEventListener('change',updateUniverse);form.elements.watchlist_id.addEventListener('change',updateUniverse);updateUniverse();
...
def create_app(service: PutScanService | None = None, *, base_url: str = "https://localhost:5000/v1/api", status_transport: object | None = None,
               ...
               workspace_store: UserWorkspaceStore | None = None, user_id: str = "local",
               ticker_groups=None):
    scanner = service or PutScanService()
    store = workspace_store or UserWorkspaceStore()
    try:
        store.watchlists_for(user_id)
    except KeyError:
        store.add_user(User(user_id, "Usuario local"))
    universes = TickerUniverseManager(store, ticker_groups)
    ...
    def application(environ, start_response):
        ...
        if environ["REQUEST_METHOD"] == "POST":
            try:
                ...
                universe = universes.resolve(values.get("universe_source", "manual"), user_id=user_id,
                    group=values.get("group", ""), watchlist_id=values.get("watchlist_id", ""),
                    manual=values.get("ticker", ""))
                values["resolved_tickers"] = " ".join(universe.tickers)
                if (universe.source is UniverseSource.MANUAL
                        and values.get("save_watchlist") == "1"):
                    name = values.get("watchlist_name", "").strip()
                    if not name:
                        raise ValueError("La watchlist necesita nombre.")
                    universes.save_manual(user_id=user_id, watchlist_id=name, name=name,
                                          manual=values.get("ticker", ""))
                request = ScanRequest(
                    ticker=universe.tickers[0], min_dte=int(values.get("min_dte", "")),
                ...
                if hasattr(scanner, "run_universe"):
                    result = scanner.run_universe(universe.tickers, request, base_url=base_url,
                                                  allow_insecure_tls=True)
                else:
                    result = tuple(scanner.run(replace(request, ticker=ticker),
        ...
        body = render_page(values, result, error, groups=universes.groups,
                           watchlists=store.watchlists_for(user_id))
