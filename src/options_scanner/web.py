...
def _directional_distance(value: float, kind: str) -> str:
    direction = "por debajo del precio" if kind == "support" else "por encima del precio"
...
def _candidate_technical_context(candidate, context=None) -> str:
    """Render context stored on the candidate, isolated from other tickers."""
    if context and context.period == HistoricalPeriod.MULTI:
        relationship, confluence, horizons, strike_relation = _confluence_strike_values(candidate, context)
        item = relationship.confluence
        if item is None:
            return '<span class="na technical-compact">Sin confluencia relevante</span>'
        origins = ", ".join(f"{origin.period.value.upper()}: {origin.zone.strength}"
                            for origin in item.origins)
        return (f'<details class="candidate-technical"><summary><span class="technical-compact">'
                f'{escape(relationship.position_label)} · {horizons}</span></summary>'
                f'<p>{confluence} · {escape(strike_relation)} · {escape(origins)}</p></details>')
...
def _summary(result: ScanResult | None) -> str:
    ...
    items = (
        ...
        ("No evaluados", s.failed_contracts + s.unresolved_contracts_timeout),
        ("Market data incompleta/no disponible", s.incomplete),
        ("Rechazados por DTE", s.rejected_dte),
        ("Rechazados por distancia al strike", s.rejected_margin),
        ("Rechazados por |Delta|", s.rejected_delta),
        ("Rechazados por IV mínima", s.rejected_iv),
        ("Rechazados por Theta short mínimo", s.rejected_theta),
...
def _interpretation(result: ScanResult | None) -> str:
    ...
    if summary.timed_out:
        ...
        not_evaluated = summary.failed_contracts + summary.unresolved_contracts_timeout
        messages.append((
            "warning",
    ...
    reasons = (
        (summary.rejected_dte, "contratos quedaron fuera del rango DTE configurado."),
        ...
        (summary.rejected_iv, "contratos no alcanzaron la IV mínima configurada."),
        (summary.rejected_theta, "contratos no alcanzaron el Theta short mínimo configurado."),
        (summary.incomplete, "contratos no pudieron evaluarse completamente por market data incompleta/no disponible."),
    )
    ...
    discarded = (
        ("DTE", summary.rejected_dte),
        ("Distancia mínima al strike", summary.rejected_margin), ("|Delta| mínimo/máximo", summary.rejected_delta),
        ("IV mínima", summary.rejected_iv), ("Theta short mínimo", summary.rejected_theta),
        ("Market data incompleta/no disponible", summary.incomplete),
        ("No evaluados", summary.failed_contracts + summary.unresolved_contracts_timeout),
    )
    discarded_rows = "".join(
    ...
    discarded_details = "".join(
        "<li>" + escape(
            f"{item.ticker} · {item.expiration.isoformat() if item.expiration else 'vencimiento no resuelto'} "
            f"· PUT ${item.strike:g} · {'; '.join(item.reasons)}"
        ) + "</li>"
        for item in summary.discarded_contracts
    )
    if not discarded_details:
        discarded_details = "<li>No hay motivos contractuales disponibles.</li>"
    return (
        ...
        f'<dl>{discarded_rows}</dl><ul class="discard-reasons">{discarded_details}</ul></details></section>'
    )
...
def _technical_chart(result: ScanResult | None, *, lazy: bool = False) -> str:
    ...
    if context and context.period == HistoricalPeriod.MULTI:
        ...
        for item in context.confluences:
            ...
            origins = "".join(f'<li>{origin.period.value.upper()}: ${origin.zone.lower:.2f}–'
    ...
    for label, zone in [(f"S{i}", z) for i,z in enumerate(supports,1)] + [(f"R{i}", z) for i,z in enumerate(resistances,1)]:
        ...
        rows.append(f'<tr><th scope="row">{label}</th><td>${zone.lower:.2f}–${zone.upper:.2f}</td><td>{_directional_distance(distance, zone.kind.value)}</td><td>{zone.strength.capitalize()}</td><td>{zone.contacts}</td><td>{sessions} sesiones</td></tr>')
...
def _multi_screener(items: tuple[tuple[str, ScanResult | None, str | None], ...],
    ...
    for ticker, result, item_error in items:
        if result is None:
            ...
            if multi_mode and context:
                if best:
                    ...
                    nearest = min(support_confluences,
        ...
        opener = (f'<td class="detail-launch"><button type="button" class="detail-trigger chevron" '
                  f'aria-label="Ver detalle de {escape(ticker)}"><span aria-hidden="true">›</span></button></td>')
...
def render_page(values: dict[str, str] | None = None, result: ScanResult | None = None, error: str | None = None,
    ...
    watchlist_rows = "".join(
        ...
        <div class="watchlist-actions"><button name="action" value="watchlist_update" type="submit">Guardar</button>
        <button name="action" value="watchlist_delete" type="submit" class="danger">Eliminar</button></div></form>'''
    ...
    history_help = ("3M / 6M / 1A: analizan soportes y resistencias utilizando únicamente ese horizonte histórico. "
                    "Multi: analiza independientemente 3M, 6M y 1A y busca confluencias entre las zonas técnicas "
                    "detectadas en al menos dos horizontes. Una confluencia 3/3 significa que los tres horizontes "
                    "presentan una zona solapada; 2/3, que coinciden dos de los tres. El horizonte histórico afecta "
                    "únicamente al contexto técnico del underlying. No modifica DTE, Delta, Theta, IV ni los demás "
                    "criterios del screening de opciones.")
    universe_help = ("Esta versión está diseñada para acciones de EE. UU. que dispongan de opciones accesibles "
                     "mediante Interactive Brokers. Otros mercados, ETFs, índices y otros tipos de activo no "
                     "forman parte actualmente del universo soportado.")
...
form{{display:flex;flex-wrap:wrap;gap:1rem;align-items:end;background:white;padding:1.25rem;border-radius:10px;box-shadow:0 2px 8px #0001}}label{{display:grid;gap:.35rem;font-weight:600}}input{{padding:.55rem;border:1px solid #aab3c5;border-radius:5px;width:9rem}}button{{background:#2358d5;color:white;border:0;border-radius:5px;padding:.7rem 1.4rem;font-weight:700;cursor:pointer}}button:disabled{{cursor:not-allowed;opacity:.65}}button.danger{{background:#a52832}}.mode{{display:flex;align-items:center;gap:.4rem}}.mode input{{width:auto}}.watchlists{{background:white;padding:.55rem .7rem;border-radius:10px}}.watchlist-row{{display:grid;grid-template-columns:minmax(9rem,.7fr) minmax(16rem,2fr) 10.5rem;gap:.45rem;align-items:end;box-shadow:none;border-top:1px solid #dde2ea;border-radius:0;padding:.4rem 0}}.watchlist-row label{{gap:.15rem;font-size:.78rem;line-height:1}}.watchlist-row input{{box-sizing:border-box;width:100%;height:2.15rem;padding:.4rem}}.watchlist-actions{{display:grid;grid-auto-flow:column;grid-auto-columns:max-content;gap:.45rem;align-items:end}}.watchlist-row button{{height:2.15rem;padding:.4rem .75rem;white-space:nowrap}}.watchlist-row input[name="watchlist_tickers"]{{width:100%}}
        ...
        if environ["REQUEST_METHOD"] == "POST":
            try:
                ...
                safe_validation_messages = {
                    ...
                    "Ya existe una watchlist con ese nombre.",
                }
                message = str(exc)
